...
- **Master/host-role emulation only** (UART TX/RX as a peer would do,
  SPI master, I2C master, JTAG/SWD debug-host) -- **slave/device
  emulation is out of scope for now, not dropped.** The brief itself
  doesn't say master-only, and this was never stated explicitly until a
  deep design audit flagged the gap: the 100-scenario firmware audit
  only ever validated master-mode programs, and slave-mode has a
  fundamentally different timing constraint -- the clock (SCLK, or I2C
  SCL from a real master) is externally driven, not something this
  design generates and paces itself, so `DELAY`-based self-pacing
  doesn't apply and firmware would need to react within a clock edge it
  doesn't control. Deferred, revisit later if time permits -- same
  "last resort" treatment as 10M Ethernet ([protocol_timing.md](protocol_timing.md)),
  not a redesign if it does get tackled (the opcode set doesn't obviously
  block it, the timing budget just hasn't been worked out).
