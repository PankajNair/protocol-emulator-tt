...
- **Illegal-opcode detection in `core.v`**: `illegal_op_flag` sets iff
  ...
  and friends).
- **Pin direction never contended** (`pin_ctrl.v`): `uio_oe[n]` only
  ever asserts for a `pin_index` whose sticky mode is push-pull or
  open-drain-driving-low, never input. Genuinely undefined until an
  adversarial review caught the missing per-pin mode concept entirely
  -- now that `SET`'s sticky drive-mode field is locked (`docs/isa.md`
  `SET` row) and implemented (`pin_ctrl.v`'s `mode`/`drv` arrays), this
  is a real, checkable invariant. `DEBUG_PORTS` already has the
  unpacked-array entries this needs.
- **Reset brings `core.v` back to a known fetch state within N cycles.**

## Out of scope for now

The sibling RISC-V project's formal setup is one piece of a much larger
verification environment (`files/VAL.md`: coverage model, stimulus
generation, mutation testing gate, golden model, subagent contracts).
This project doesn't have a coverage model, mutation testing, or a
golden reference model yet -- formal verification is the concrete,
working nucleus for now; the rest is future work, not silently assumed
to exist.
